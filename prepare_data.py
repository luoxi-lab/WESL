...
def subsample(i_index,j_index,img,img2):
    for i in range(i_index):
        for j in range(j_index):
            if((i+j)%2==0):
                img[i,j] = 0
                img2[i,j]=0
    return np.concatenate((img.reshape(1,512,640),img2.reshape(1,512,640)),axis=0)
    ...
    for i in range(i_index):
        for j in range(j_index):
            if random.randint(1,2)==1:
                ...
                img2[i,j]=0
                img3[i,j]=0
    return np.concatenate((img.reshape(1,512,640),img2.reshape(1,512,640),img3.reshape(1,512,640)),axis=0)
...
def main():
    ...
    parser.add_argument("--output_folder", default='./datasets')
    parser.add_argument("--noise_type", default="gaussian")
    parser.add_argument("--std", default=1.0)
    parser.add_argument("--lam", default=30)
    parser.add_argument("--num_sim", default=6)
    parser.add_argument("--patch_size", default=5)
    parser.add_argument("--max_files", default=1000)
    ...
    parser.add_argument("--data_num", default=512)
    ...
    args.output_file = "{}/seg_0610gus_{}{}_ps{}_ns{}_kahu418_lmdb".format(args.output_folder, args.noise_type,
    ...
    if args.config_file is None:
        ...
        img = data_in_test.reshape(args.data_num,512,640,1) 
        

    
    data_noise = np.array(img).astype(np.float32)
    ...
    for idx, key in tqdm_iter:
        ...
        img_noise_sim_1 = compute_sim_images(img_noise, patch_size=3, num_select=args.num_sim,img_ori=img_noise_norm)
        img_noise_sim_2 = compute_sim_images(img_noise, patch_size=5, num_select=args.num_sim,img_ori=img_noise_norm)
        img_noise_sim_3 = compute_sim_images(img_noise, patch_size=7, num_select=args.num_sim,img_ori=img_noise_norm)

   
        img_noise_sim1_1 = img_noise_sim_1[random.randint(0,args.num_sim-1),:,:,:].reshape(512,640)
        img_noise_sim2_1 = img_noise_sim_2[random.randint(0,args.num_sim-1),:,:,:].reshape(512,640)
        img_noise_sim3_1 = img_noise_sim_3[random.randint(0,args.num_sim-1),:,:,:].reshape(512,640)
        
        img_noise_sim1_2 = img_noise_sim_1[random.randint(0,args.num_sim-1),:,:,:].reshape(512,640)
        img_noise_sim2_2 = img_noise_sim_2[random.randint(0,args.num_sim-1),:,:,:].reshape(512,640)
        img_noise_sim3_2 = img_noise_sim_3[random.randint(0,args.num_sim-1),:,:,:].reshape(512,640)
        ...
        if randidx==1:
            ...
            img_noise_sim2 = img_noise_sim1_2.copy()
        elif randidx==2:
            img_noise_sim1 = img_noise_sim2_1.copy()
            img_noise_sim2 = img_noise_sim2_2.copy()
        else:
            img_noise_sim1 = img_noise_sim3_1.copy()
            img_noise_sim2 = img_noise_sim3_2.copy()

           
        print(np.array_equal(img_noise_sim1,img_noise_sim2))

        sim1_temp1 = img_noise_sim1.copy()
        sim1_temp2 = img_noise_sim1.copy()
        sim2_temp1 = img_noise_sim2.copy()
        sim2_temp2 = img_noise_sim2.copy()
        

 
        con1 = subsample(512,640,sim1_temp1,sim2_temp1)
        
        noise1 = con1[1].reshape(512,640)  
        noise2 = sim2_temp2 - noise1     
        
        sim1 = con1[0].reshape(512,640)   
        sim2 = sim1_temp2 - sim1
        
       
        pair1 = noise1 + sim2
        pair2 = noise2 + sim1

        pair1 = pair1.reshape(1,512,640,1)
        pair2 = pair2.reshape(512,640,1)
        print(img_noise_sim1.shape,img_noise_sim2.shape)
    ...
    pickle.dump(meta_info, open("{}_seg_0610gus_meta_kahu418_info.pkl".format(args.output_file), "wb"))
